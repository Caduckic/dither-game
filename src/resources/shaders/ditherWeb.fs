...
// Flattened 1D dither array (replacing dither[8][8])
...
void main()
{
    ...
    int bayerValue = dither[(pixelPos.y % 8) * 8 + (pixelPos.x % 8)];

    // Adjust the color with the threshold and Bayer matrix
    vec3 adjustedColor = texelColor.rgb;
    adjustedColor += (float(bayerValue) - 32.0 + threshold) / 128.0;
